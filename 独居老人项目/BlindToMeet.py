...
def JS_2():
    ...
    while sign:
        ...
        # ser.write(packed)
        # 高、低地址、信道
        SendData.SendPacket(b'NEWPOI', 1, 0x00, 0x00, Ck[c], packed, t)
        time.sleep(2)
...
def SendPacket(form, num, num1, num2, num3, add=None, isfall=0, isCure=False):
...
if __name__ == '__main__':
    ...
    GPIO.output(27, GPIO.HIGH)
    ser.write(b'\xC2\x05\x01\x02')  # 设置信道
    while True:
        buf = ser.read(4)
        if buf == b'\xC1\x05\x01\x02':
            GPIO.output(27, GPIO.LOW)
            break
        else:
            ser.write(b'\xC2\x05\x01\x02')

    packed = b'\x01\x01\x01'
    while True:
        time.sleep(10)
        SendData(b'STOPTX', 1, 0x00, 0x00, 0x02, packed)
