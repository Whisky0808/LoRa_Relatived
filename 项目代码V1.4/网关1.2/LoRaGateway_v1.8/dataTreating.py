...
class DataTreating:
    ...
    def SendData(self, form, num, num1=0, num2=0, num3=0, isFall=False, isCure=False):
        """发送数据"""
    ...
    def Treat(self):
        ...
        if type(self.RX_Packet) == tuple:
            if self.RX_Packet[0] == b'SENSOR':
                if self.RX_Packet[5] and self.RX_Packet[6]:
                    self.SendData(b'reBoth', self.RX_Packet[1], 0, 0, 0, True, True)
                elif self.RX_Packet[5]:
                    self.SendData(b'reFall', self.RX_Packet[1], 0, 0, 0, True, False)
                elif self.RX_Packet[6]:
                    self.SendData(b'reCure', self.RX_Packet[1], 0, 0, 0, False, True)
